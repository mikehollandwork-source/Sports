...
def load_ledger() -> dict:
    ...
    if "plays" in old or ("picks" in old and "leans" in old):
        ...
        # v12: the separate display-only Vegas book is removed (user call) - one
        # picks record only. The book_needs read still drives the fade gate; it's
        # just no longer tracked as its own W-L.
        old.pop("vegas", None)
        return old
...
def windowed_records(book: dict, today: dt.date) -> list[tuple[str, tuple[int, int, float]]]:
    """W-L-units for a book over Today / Yesterday / Week / Month / YTD. Today =
    games settled with TODAY's date only (0-0 until today's games go final);
    Yesterday = only the prior day's settled plays; Week/Month/YTD are calendar
    windows (since Monday / since the 1st / since Jan 1) ending today. Empty list
    when the book has no bets at all."""
    e = book["entries"]
    if not e:
        return []
    ...
    windows = [
        ...
        (f"Yesterday ({yesterday_s})", [x for x in e if x["date"] == yesterday_s]),
